...
class core:
    ...
    def post_method(self):
        ...
        for form in forms:
            ...
            if form["method"].lower().strip() == "post":
                ...
                req = self.session.post(urljoin(self.url, action), data=keys, timeout=10)
    ...
    def get_method_form(self):
        ...
        for form in forms:
            ...
            if form["method"].lower().strip() == "get":
                ...
                req = self.session.get(urljoin(self.url, action), params=keys, timeout=10)
                if self.payload in req.text:
            ...
            if url.startswith("http://") is False or url.startswith("https://") is False or url.startswith("mailto:") is False:
                ...
                if query != "":
                    ...
                    if not url.startswith("mailto:") and not url.startswith("tel:"):
                        _respon = self.session.get(test, verify=False, timeout=10)
                        if self.payload in _respon.text or self.payload in self.session.get(query_all, timeout=10).text:
    ...
    def main(self, url, proxy, headers, payload, cookie, method=2):
        ...
        try:
            ctr = self.session.get(url, timeout=10)
            self.body = ctr.text
        except Exception as e:
